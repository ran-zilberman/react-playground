...
describe('<BoDomainServicesPaneContainer/>', () => {
    ...
    const jsonInspectorClass = '.json-inspector';
    ...
    it('should display json viewer after fetching data from the server - DomainGet', done => {
        fetchMock.post(ServerUri.GET_DOMAIN_DATA, responseParams(ContentType.JSON_CONTENT_TYPE, dummyResultJsonObject()));
        simulateClickAndAssertClassExists(jsonInspectorClass, getDomainDataClassName, done)
    });

    // it('should display json viewer after fetching data from the server - DomainTransfer', () => {
    //
    // })
});
